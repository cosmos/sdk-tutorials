...
type Keeper struct {
	...
	allNamesStoreKey sdk.StoreKey // Unexposed key to access list of all names from sdk.Context
...
func NewKeeper(coinKeeper bank.Keeper, allNamesStoreKey sdk.StoreKey, namesStoreKey sdk.StoreKey, ownersStoreKey sdk.StoreKey, priceStoreKey sdk.StoreKey, cdc *codec.Codec) Keeper {
	return Keeper{
		coinKeeper:     coinKeeper,
		allNamesStoreKey:  allNamesStoreKey,
...
// GetNames - returns the string that the name resolves to
func (k Keeper) GetNames(ctx sdk.Context) string {
	store := ctx.KVStore(k.allNamesStoreKey)
	bz := store.Get([]byte("*"))
	return string(bz)
}
...
// NewName - adds the name when it is new
func (k Keeper) NewName(ctx sdk.Context, name string) {
	store := ctx.KVStore(k.allNamesStoreKey)
	bz := store.Get([]byte("*"))
	bzstring := string(bz)
	var strs []string
