...
And we store the board at `/store/board`, and return it in the response's `Value`, when requested via the [`Query`](https://github.com/tendermint/abci/blob/master/specification.md#query) command at `Path = "/store/board"`. If and when we need to reinstantiate the board state out of its serialized form, we [can call](https://github.com/batkinson/checkers-go/blob/a09daeb/checkers/checkers.go#L331):
...
[That's](https://github.com/tendermint/abci/blob/master/specification.md#initchain) where our only game is initialized. **TBD** We configure Tendermint to pass the initial serialized game state to the application inside `/store`. Notice how our application also has to handle the list of validators sent by Tendermint. Let's gloss over this _detail_.
...
[This command](https://github.com/tendermint/abci/blob/master/specification.md#beginblock) instructs the application to load its state at the right place. More precisely, inside `Header (struct{})`, as per its [detailed definition](https://github.com/tendermint/abci/blob/master/specification.md#header), it contains:
...
In case the header has omitted the `AppHash`, the application should work off the last state it has arrived at.
...
That's where Tendermint [asks](https://github.com/tendermint/abci/blob/master/specification.md#checktx) our application whether the transaction is worth keeping at all. Because we want to simplify to the maximum, we only concern ourselves with whether there is a valid move in the transaction.

* First, is there a move? So we check whether there are 4 `int` in the serialized information.
* Second, ABCI asks us to do _stateful but light-weight checks_. Luckily, our implementation [has]():
    > `func (game *Game) ValidMove(src, dst Pos) bool`

    Therefore we can return the result of this check on the currently loaded state of the board.

#### `DeliverTx`: a transaction is added and needs to be processed

Now, a pre-checked transaction [is delivered](https://github.com/tendermint/abci/blob/master/specification.md#delivertx). It is a matter of applying it to the latest board state. Conveniently, we can [call](https://github.com/batkinson/checkers-go/blob/a09daeb/checkers/checkers.go#L274):

> `func (game *Game) Move(src, dst Pos) (captured Pos, err error)`

And handle the error if necessary. If it went ok, then we keep the new board state in memory, ready for the next delivered transaction.

We can also choose to let it know what interesting information should be indexed via `Tags ([]cmn.KVPair)` in the response. The intent of these returned values is to allow a fast search across blocks for values of relevance. For the sake of the exercise, let's imagine that we want to index the following information in key / value pair (`KVPair`) form:

```
// Some self-explanatory metadata about the move just executed:
whitePlayed: bool,
isJump: bool,
madeKing: bool,
hasCaptured: bool,
hasCapturedKing: bool,
isWinning: bool,
// Some metadata about the resulting board:
whiteCount: uint32,
whiteKingCount: uint32,
redCount: uint32,
redKingCount: uint32
```

Or it could be grouped more coarsely:
...
If you come from the Ethereum world, you will recognize these as _topics_ in the transaction receipt logs, a.k.a. indexed Solidity events.
...
Now is [the time](https://github.com/tendermint/abci/blob/master/specification.md#endblock) to let Tendermint know about some things. Setting aside the validator thing, we need to let it know what should be indexed. The intent of these returned pairs is to allow a fast search across blocks for values of relevance. In our case, we have only checkers moves, so it is not very clear what could be interesting to be searched at a later date. For the sake of the argument, let's assume that we want to tally what happened in the block. So we return these pairs:

```
whiteCapturedCount: uint32,
whiteKingCapturedCount: uint32,
redCapturedCount: uint32,
redKingCapturedCount: uint32,
```

#### `Commit`: our work here is done

The block that has previously ended has now been confirmed, and the application needs to save its state to storage, to its database. And return, as `Data ([]byte)`, the Merkle root hash of the blockchain's state, which, as you recall, includes `/store/board`. As mentioned in [the doc](https://github.com/tendermint/abci/blob/master/specification.md#commit), this hash needs to be deterministic after the sequence of the same `BeginBlock`, the same `DeliverTx`'s in the same order, and the same `EndBlock`.
