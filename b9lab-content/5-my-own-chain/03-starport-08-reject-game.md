...
As a prerequisite to dive right into implementing game rejection, make sure you know how to [create a message](./03-startport-04-create-message.md) with Starport, and code [its handling](./03-startport-05-create-handling.md). This section does not aim to repeat what can be learn in earlier sections.
...
A new rule of the game should be that a player cannot reject a game once they begin to play. However, as of now, when loading a `StoredGame` from storage, you have no way of knowing whether a player already played or not. There are many way to remediate this. Let's pick an effortless one, where you add a new field to the `StoredGame`. Let's call it `MoveCount`. So, in `proto/checkers/stored_game.proto`:
...
As a convenience, and to follow Cosmos SDK conventions, you declare the following new errors:
