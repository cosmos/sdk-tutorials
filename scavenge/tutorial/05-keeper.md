...
After using the `scaffold` command you should have a boilerplate `Keeper` at `./x/scavenge/internal/keeper/keeper.go`. It contains a basic keeper with references to basic functions like `Set`, `Get` and `Delete`. Some of these methods are lowercase as they should not be exported or accessed from other locations.

Our keeper stores all our data for our module. Sometimes a module will import the keeper of another module. This will allow state to be shared and modified across modules. Since we are dealing with persistent data relative to coins in our module as bounty rewards, we will need to access the `bank` module's keeper (which we call `CoinKeeper`). Look at our completed `Keeper` and you can see where the `bank` keeper is referenced and how `Set`, `Get` and `Delete` are expanded:
...
You may notice the use of `types.ScavengePrefix` and `types.CommitPrefix`. These are defined in a file called `./x/scavenge/internal/types/key.go` and help us keep our `Keeper` organized. The `Keeper` is really just a key value store. That means that, similar to an `Object` in javascript, all values are referenced under a key. To access a value, you need to know the key under which it is stored. This is a bit like a UID.

When storing our `Scavenge`s we use the key of the `SolutionHash` as a unique ID, for `commits` we use the key of the `SolutionScavengeHash`. However since we are storing these two datatypes in the same location, we may want to distinguish between the types of keys. Since hash commits look the same, whether they are a `SolutionHash` or a `SolutionScavengeHash` we add prefixes so we can recognize which is which. For `Scavenge` we add the string `sk-` and for `Commit` we add the string `ck-`. You should add these to your `key.go` file so it looks as follows:
```go
package types
...
Sometimes you will want to access a `Commit` or a `Scavenge` directly by their key. That's why we have the methods `GetCommit` and `GetScavenge`. However, sometimes you will want to get all `Scavenge`s or all `Commit`s. To do this we use an **Iterator** called `KVStorePrefixIterator`. This utility comes from the `sdk` and iterates over a key store, like the one we're using to store `Commit`s and `Scavenge`s. If you provide a prefix, it will only iterate over the keys that contain that prefix. Since we have prefixes defined for our `Scavenge` and our `Commit` we can use them here to only return our desired data types.

---

Now that you've seen the `Keeper` where every `Commit` and `Scavenge` are stored, we need to connect the Messages to the Storage. This process is called _handling_ the messages and is done inside the `Handler`.

Let's take a look [here]("./06-handler.go").