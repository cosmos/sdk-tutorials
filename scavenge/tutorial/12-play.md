...
Your application is running! That's great but who cares unless you can play with it. The first command you will want to try is createing a new scavenge. Since our user `me` has way more `foo` token than the user `you`, let's create the scavenge from their account.
...
Since we know the solution to this question and since we have another user at hand that can submit it, let's begin the process of commiting and revealing the solution.
...
```
Notice that again we don't need to include the actual hash as that is taken care of for us by the CLI. We do need to include the account address of who is asking though, since this is a query and does not require us to sign anything with our account which would reveal our address. Hopefully you should see somehing like:
```json
{
  "scavenger": "cosmos1m9pxr3nrra2cl07kh8hzdty5x0mejf44997f79",
  "solutionHash": "2f9457a6e8fb202f9e10389a143a383106268c460743dd59d723c0f82d9ba906",
...
```
This confirms that your commit was successfully subitted and is awaiting the follow-up reveal. To make that command let's first check the `--help` screen using `scavengeCLI tx scavenge revealSolution --help`. This should show the following screen:
...
```
Trying to solve it again will throw an error, same as trying to submit another scavenge with the same solution.

Thanks for joining me in building a deterministic state machine and using it as a game. I hope you can see that even such a simple app can be extremely powerful as it contains a form of digital scarcity. 

Try expanding on the capabilities of this application, for example:
* Allow the `Creator` of a `Scavenge` to edit or delete a scavenge.
* Create a query that lists all commits.

If you're interested in learning more about the Cosmos SDK check out the rest of our docs and sign up for our newsletter so you hear about new talks and tutorials. Topics to look out for in future tutorials are:
* Communication between applications
* Digital Collectibles (NFTs)
* Using the Ethereum Virtual Machine within an application

If you have any questions or comments feel free to open an issue on this [github](https://github.com/cosmos/sdk-tutorials). If you'd like to stay in touch with me follow me on github at [@okwme](https://github.com/okwme) or twitter at [@billyrennekamp](https://twitter.com/billyrennekamp).