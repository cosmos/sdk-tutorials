...
1. The **play message**, which means implicitly accepting the challenge when playing for the first time. If you create it with Ignite CLI, use:
    ...
    To jump straight to the corresponding part of the coding exercise, head to [Add a Way to Make a Move](/hands-on-exercise/1-ignite-cli/6-play-game).
...
* Handling the cancellation in ABCI's `EndBlock` (or rather its equivalent in the Cosmos SDK) without any of the players having to trigger the cancelation. For the guided coding exercise on this part, head straight to [Auto-Expiring Games](/hands-on-exercise/2-ignite-cli-adv/4-game-forfeit).
