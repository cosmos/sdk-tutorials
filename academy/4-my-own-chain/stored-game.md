...
Now that you have added some code on top of what Ignite CLI created for you, it would be welcome to add unit tests. You are not going to add code to test the code generated by Ignite CLI as your project is not the right place to _test the framework_.

On the other hand, Ignite CLI added some unit tests of its own. Run those for the keeper with:
...
A good first place is to test that the default genesis is created as expected. Beside `x/checkers/types/genesis.go`, create a new `genesis_test.go` with:
...
And to run it, use `go test` with the package name:
...
Alternatively, you can call it from the folder itself:
...
You want your tests to pass when everything is ok, but you also want them to fail when something is wrong. Make sure your new test fails by changing `uint64(1)` into `uint64(2)`. You should get a:
...
It is a bit overwhelming but the meat of it is that:
...
Your first unit test is a standard Go unit test. So if you use an IDE like Visual Studio Code it is ready to assist you and run the test in debug mode. Next to the function name, you see a small green tick, and if you hover below, a faint red dot appears.

![Go test debug button in VSCode](/go_test_debug_button.png)

This red dot is a potential breakpoint. Add one on `DefaultGenesis()` line. The dot is now bright and stays there.

![Go test breakpoint placed](/go_test_debug_breakpoint.png)

Right-click on the green tick, and choose to <kbd>Debug Test</kbd>. It may ask you to install a package. Accept. After some time it stops at the breakpoint and shows you the current variables and a panel for stepping actions:

![Go test stopped at breakpoint](/go_test_debug_stopped_at_breakpoint.png)

Given how this test is written, there is unfortunately nothing much to show. If you are struggling with a test, then it pays to create separate variables in order to inspect them in debug. From there, it is your regular step-by-step debugging process. If you are not familiar with it, [this online tutorial](https://www.digitalocean.com/community/tutorials/debugging-go-code-with-visual-studio-code) is as good as any.

### More unit tests

With a simple yet successful unit test, you add more consequential ones testing the helper methods. Since you are going to repeat some actions, it is worth adding a reusable function:
...
With that, you can test the function to get the creator's address. One test for the happy path, and another for the error:
...
And you can do the same for [`Black`](https://github.com/cosmos/b9-checkers-academy-draft/blob/3c2dd1c/x/checkers/types/full_game_test.go#L48-L65) and [`Red`](https://github.com/cosmos/b9-checkers-academy-draft/blob/3c2dd1c/x/checkers/types/full_game_test.go#L67-L84).
