...
Explore the low-level details of what it means to identify a token received using IBC to your account, find out which chain the token came from, contact that chain, and verify that it exists and is still running.

Imagine that you've received a new `ibc/` token on a blockchain:
...
In Cosmos SDK, the Gaia daemon `gaiad` runs a full node of the gaia application and provides a gRPC interface. The default port is  9090. In this tutorial, you will learn to query this interface directly. 
...
So far we understand on a high level what needs to be done. But what is actually going on under the hood? How is `gaiad` getting all that data, and how can you access this data from another SDK or programming language, for example CosmJS?
...
Many parts are still under development, but the gist is that you can ensure you are talking to the correct chain after you:
